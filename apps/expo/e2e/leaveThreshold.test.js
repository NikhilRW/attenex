...
async function createLectureViaUI() {
  ...
  await element(by.id("TEACHER_DASHBOARD_SCREEN.HEADER_SECTION.HEADER_TEXT_CONTAINER")).swipe(
    "down",
    "slow",
    0.4,
    0.1,
...
async function joinLectureViaUI() {
  ...
  await waitFor(
    element(by.text("Location tracking started. Wait for class to end, then verify attendance.")),
  )
    .toBeVisible()
    .withTimeout(10000);
  await element(by.text("Ok")).tap();
...
async function submitAttendanceViaUI(pass) {
  ...
  await waitFor(element(by.id(PASSCODE_INPUT)))
    .toBeVisible()
    .withTimeout(10000);
...
describe("Location & Attendance Flow", () => {
  ...
  it(
    "marks Present when student stays in range across lecture",
    async () => {
  ...
  it.skip(
    "marks Incomplete when student leaves before lecture end",
    async () => {
